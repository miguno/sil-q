...
static errr Term_clear_gtk(void)
{
    ...
    /* Clear the area */
    gdk_draw_rectangle(
        td->pixmap, td->drawing_area->style->black_gc, 1, 0, 0, width, height);

    /* Copy it to the window */
    gdk_draw_pixmap(td->drawing_area->window, td->gc, td->pixmap, 0, 0, 0, 0,
        width, height);
...
static errr Term_wipe_gtk(int x, int y, int n)
{
    ...
    gdk_draw_rectangle(td->pixmap, td->drawing_area->style->black_gc, TRUE,
        x * td->font_wid, y * td->font_hgt, n * td->font_wid, td->font_hgt);

    /* Copy it to the window */
    gdk_draw_pixmap(td->drawing_area->window, td->gc, td->pixmap,
        x * td->font_wid, y * td->font_hgt, x * td->font_wid, y * td->font_hgt,
...
static errr Term_text_gtk(int x, int y, int n, byte a, cptr s)
{
    ...
    for (i = 0; i < n; i++)
    {
        gdk_draw_text(td->pixmap, td->font, td->gc, (x + i) * td->font_wid,
...
static errr Term_curs_gtk(int x, int y)
{
    ...
    gdk_draw_rectangle(td->pixmap, td->gc, FALSE, x * td->font_wid,
        y * td->font_hgt, td->font_wid - 1, td->font_hgt - 1);
...
static void hook_quit(cptr str) { gtk_exit(0); }
...
static void change_font_event_handler(GtkWidget* widget, gpointer user_data)
{
    ...
    gtk_font_selection_dialog_set_filter(
        GTK_FONT_SELECTION_DIALOG(font_selector), GTK_FONT_FILTER_BASE,
        GTK_FONT_ALL, NULL, NULL, NULL, NULL, spacings, NULL);
...
static void open_event_handler(GtkButton* was_clicked, gpointer user_data)
{
    ...
    else
    {
        ...
        gtk_signal_connect(
            GTK_OBJECT(GTK_FILE_SELECTION(file_selector)->ok_button), "clicked",
            file_ok_callback, (gpointer)file_selector);
    ...
    GtkWidget* widget, GdkEventKey* event, gpointer user_data)
{
     ...
     */
    if ((event->state == GDK_SHIFT_MASK) && (event->keyval >= GDK_KP_0)
    ...
    /* Build the macro trigger string */
    strnfmt(msg, sizeof(msg), "%c%s%s%s%s_%X%c", 31, mc ? "N" : "",
        ms ? "S" : "", mo ? "O" : "", mx ? "M" : "", event->keyval, 13);
    ...
    GtkWidget* widget, GdkEventExpose* event, gpointer user_data)
{
    ...
    if (pixmap)
    {
        ...
        gdk_draw_pixmap(widget->window, td->gc, pixmap, event->area.x,
...
static void init_gtk_window(term_data* td, int i)
{
    ...
    if (main_window)
    {
        ...
        /* Register callbacks */
        gtk_signal_connect(
            GTK_OBJECT(file_exit_item), "activate", quit_event_handler, NULL);
        gtk_signal_connect(
            GTK_OBJECT(file_new_item), "activate", new_event_handler, NULL);
        gtk_signal_connect(
            GTK_OBJECT(file_open_item), "activate", open_event_handler, NULL);
    ...
    /* Clear the pixmap */
    gdk_draw_rectangle(td->pixmap, td->drawing_area->style->black_gc, TRUE, 0,
        0, td->cols * td->font_wid, td->rows * td->font_hgt);
