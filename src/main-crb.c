...
void fsetfileinfo(cptr pathname, OSType fcreator, OSType ftype)
{
    ...
    if (path_to_spec(pathname, &spec) != noErr)
        return;
...
#pragma unused(fd)
    ...
    if (get_modification_time(buf, &txt_stat))
        return (-1);
    ...
    /* Obtain modification time */
    if (get_modification_time(buf, &raw_stat))
        return (-1);

    /* Ensure the text file is not newer than the raw file */
    if (txt_stat > raw_stat)
        return (-1);
...
static void term_data_check_font(term_data* td)
{
    ...
    for (i = 33; i < 127; i++)
    {
        ...
        if (td->font_wid != CharWidth(i))
            td->font_mono = FALSE;

        /* Hack -- collect largest width */
        if (td->font_wid < CharWidth(i))
            td->font_wid = CharWidth(i);
    }
...
static void term_data_check_size(term_data* td)
{
    ...
    /* Fake mono-space */
    if (!td->font_mono || (td->font_wid != td->tile_wid)
 ...
 */
static OSErr create_gworld_from_spec(GWorldPtr* tile_gw, FSSpec* tile_spec)
{
    OSErr err;
...
static errr globe_init(void)
{
    ...
    if (!get_resource_spec(pict_id, CFSTR("png"), &pict_spec))
        return (-1);
    ...
    if ((newPictH = GetPicture(pict_id)) == 0)
        return (-1);
...
static void load_sounds(void)
{
    ...
    for (i = 1; i < SOUND_MAX; i++)
    {
        ...
        name = CFStringCreateWithCString(
            NULL, angband_sound_name[i], kTextEncodingUS_ASCII);
        ...
        err = NewMovieFromFile(
            &movie, file_id, &res_id, movie_name, newMovieActive, NULL);
...
static void cleanup_sound(void)
{
    ...
    for (i = 1; i < SOUND_MAX; i++)
    {
        ...
        /* Release it */
        if (samples[i])
            ReleaseResource((Handle)samples[i]);

#else
        /* Free handle */
        if (samples[i])
            DisposeHandle((Handle)samples[i]);
...
static void Term_init_mac(term* t)
{
    ...
    /* Make the window  */
    err = CreateNewWindow(kDocumentWindowClass, wattrs, &td->r, &td->w);
    ...
    //{
    // Sil-y: I've had to turn this on for all versions, as it has always
    // affected me (10.3 -> 10.5 at least
    ...
    if (td->mapped)
    {
        TransitionWindow(td->w, kWindowZoomTransitionEffect,
...
#endif /* USE_TRANSPARENCY */
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (use_graphics && ((byte)a & 0x80) && ((byte)c & 0x80))
        {
            ...
            {
                /* Draw the picture */
                CopyBits((BitMap*)frameP->framePix, (BitMap*)*pixmap_h, &src_r,
                    &dst_r, srcCopy, NULL);
            ...
            {
                /* Draw the terrain */
                CopyBits((BitMap*)frameP->framePix, (BitMap*)*pixmap_h,
...
static void save_pref_short(const char* key, short value)
{
    ...
    if ((cf_key != NULL) && (cf_value != NULL))
    {
        ...
        CFPreferencesSetAppValue(
            cf_key, cf_value, kCFPreferencesCurrentApplication);
    }
...
static bool query_load_pref_short(const char* key, short* vptr)
{
    ...
    CFNumberGetValue(cf_value, kCFNumberShortType, vptr);
...
static void load_pref_short(const char* key, short* vptr)
{
    ...
    if (query_load_pref_short(key, &tmp))
        *vptr = tmp;
    return;
}
 ...
 * Save preferences
 */
static void save_pref_file(void) { cf_save_prefs(); }
...
static bool select_savefile(bool all)
{
    ...
    /* Oops */
    if (err != noErr)
        quit("Unable to find the folder :lib:save:");
    ...
    err = AECreateDesc(
        typeFSS, &theFolderSpec, sizeof(FSSpec), &defaultLocation);

    /* Oops */
    if (err != noErr)
        quit("Unable to allocate descriptor");
    ...
    err = NavChooseFile(&defaultLocation, &reply, &dialogOptions, NULL, NULL,
        NULL, myTypeList, NULL);
    ...
    /* Convert FSSpec to pathname and store it in variable savefile */
    refnum_to_name(savefile, savedGameSpec.parID, savedGameSpec.vRefNum,
...
static void init_menubar(void)
{
    ...
    /* The new way - loading main menu using Interface Builder services */
    {
        ...
        if (err != noErr)
            quit("Cannot find Main.nib in the bundle!");
    ...
    /* The old way - loading main menu from Resource Manager resource */
    {
        ...
        /* Whoops! */
        if (mbar == nil)
            quit("Cannot find menubar('MBAR') id 128!");
    ...
    (void)CreateNewWindow(kDocumentWindowClass, kWindowNoAttributes, &r, &tmpw);
...
static void setup_menus(void)
{
    ...
    if (initialized && td)
    {
        ...
        if (td->font_face & bold)
            CheckMenuItem(m, ITEM_BOLD, TRUE);

        /* Check the appropriate "wide-ness" */
        if (td->font_face & extend)
            CheckMenuItem(m, ITEM_WIDE, TRUE);
    ...
    if (initialized && td)
    {
        ...
        for (i = 1; i <= n; i++)
        {
            ...
            if (RealFont(td->font_id, value))
                EnableMenuItem(m, i);

            /* Check the current size */
            if (td->font_size == value)
                CheckMenuItem(m, i, TRUE);
        }
    }
    ...
    for (i = 1; i <= n; i++)
    {
        ...
        if ((i != ITEM_GRAPH) && (i != ITEM_TILEWIDTH)
            && (i != ITEM_TILEHEIGHT))
            CheckMenuItem(m, i, FALSE);
    ...
    EnableMenuItem(m, ITEM_TILEWIDTH);
    {
        ...
        if (initialized && td)
        {
            ...
            for (i = 1; i <= n; i++)
            {
                ...
                /* Enable */
                if (value >= td->font_wid)
                    EnableMenuItem(submenu, i);

                /* Check the current size */
                if (td->tile_wid == value)
                    CheckMenuItem(submenu, i, TRUE);
            }
        }
    }
    ...
    EnableMenuItem(m, ITEM_TILEHEIGHT);
    {
        ...
        if (initialized && td)
        {
            ...
            for (i = 1; i <= n; i++)
            {
                ...
                /* Enable */
                if (value >= td->font_hgt)
                    EnableMenuItem(submenu, i);

                /* Check the current size */
                if (td->tile_hgt == value)
                    CheckMenuItem(submenu, i, TRUE);
            }
        }
    }
...
static void menu(long mc)
{
    ...
    /* Relevant "term_data" */
    if (old_win)
        td = (term_data*)GetWRefCon(old_win);
    ...
    case MENU_APPLE:
    {
        ...
        if (selection == ITEM_ABOUT)
        {
            ...
            RepositionWindow(
                GetDialogWindow(dialog), NULL, kWindowCenterOnMainScreen);
    ...
    case MENU_FILE:
    {
        ...
        case ITEM_SCORE:
        {
            ...
            /* Paranoia */
            if (!initialized || character_icky || !game_in_progress
                || !character_generated)
            {
...
static OSErr CheckRequiredAEParams(const AppleEvent* theAppleEvent)
{
    ...
    aeError = AEGetAttributePtr(theAppleEvent, keyMissedKeywordAttr,
        typeWildCard, &returnedType, NULL, 0, &actualSize);

    if (aeError == errAEDescNotFound)
        return (noErr);
 ...
 */
static pascal OSErr AEH_Start(
    const AppleEvent* theAppleEvent, AppleEvent* reply, SInt32 handlerRefCon)
{
#pragma unused(reply)
...
#pragma unused(handlerRefCon)
     ...
     */
    err = AEGetNthPtr(&docList, 1L, typeFSS, &keywd, &returnedType, (Ptr)&myFSS,
 ...
 */
static pascal OSErr AEH_Reopen(
    const AppleEvent* theAppleEvent, AppleEvent* reply, long handlerRefCon)
{
...
static bool CheckEvents(int wait)
{
    ...
    case mouseDown:
    {
        ...
        case inDrag:
        {
            ...
            /* Analyze */
            GetWindowBounds((WindowRef)td->w, kWindowContentRgn, &pRect);
        ...
        case inGrow:
        {
            ...
            /* Grow the rectangle */
            if (!ResizeWindow(w, event.where, &r, NULL))
                break;
#else

            /* Grow the rectangle */
            if (!ResizeWindow(w, event.where, NULL, NULL))
                break;
...
static void hook_core(cptr str)
{
    ...
    /* Attempt to save */
    if (!save_player())
        mac_warning("Warning -- save failed!");
...
static void init_stuff(void)
{
    ...
    while (1)
    {
        ...
        err = NavChooseFolder(nil, &theReply, &dialogOptions, nil, nil, nil);
        ...
        /* Retrieve FSSpec from the reply */
        {
            ...
            err = AEGetNthPtr(&(theReply.selection), 1, typeFSS, &theKeyword,
        ...
        /* Extract textual file name for given file */
        refnum_to_name(path, theFolderSpec.parID, theFolderSpec.vRefNum,
...
int main(void)
{
    ...
    if (err != noErr)
        quit("This program requires Carbon API");
