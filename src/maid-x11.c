...
static void rd_byte(FILE* fff, byte* ip) { *ip = get_byte(fff); }
...
XImage* ReadBMP(Display* dpy, char* Name)
{
    ...
    Res = XCreateImage(dpy, visual, depth, ZPixmap, 0 /*offset*/, Data,
        infoheader.biWidth, infoheader.biHeight, 32 /*bitmap_pad*/,
        0 /*bytes_per_line*/);
    ...
    for (y = 0; y < infoheader.biHeight; y++)
    {
        ...
        for (x = 0; x < infoheader.biWidth; x++)
        {
            ...
            else
            {
                ...
                quit_fmt(
                    "Illegal biBitCount %d in %s", infoheader.biBitCount, Name);
            }
        }
    }
...
static void GetScaledRow(XImage* Im, int x, int y, int iw, int ow,
    unsigned long* redScan, unsigned long* greenScan, unsigned long* blueScan)
{
 ...
 */
static void ScaleIcon(XImage* ImIn, XImage* ImOut, int x1, int y1, int x2,
    int y2, int ix, int iy, int ox, int oy)
{
    int div;
    ...
    if (iy == oy)
    {
        ...
        {
            GetScaledRow(
                ImIn, x1, y1 + yi, ix, ox, tempRed, tempGreen, tempBlue);
            PutRGBScan(
                ImOut, x2, y2 + yi, ox, div, tempRed, tempGreen, tempBlue);
        }
    }
    else if (iy < oy)
    {
        ...
        for (yi = 0; yi <= oy; yi++)
        {
            if (getNextRow)
            {
                ...
                if (yi < oy)
                {
                    ...
                    GetScaledRow(
                        ImIn, x1, si + 1, ix, ox, nextRed, nextGreen, nextBlue);
                }
            }
            ...
            {
                tempRed[xi]
                    = (prevRed[xi] * (oy - sifrac) + nextRed[xi] * sifrac);
                tempGreen[xi]
                    = (prevGreen[xi] * (oy - sifrac) + nextGreen[xi] * sifrac);
                tempBlue[xi]
                    = (prevBlue[xi] * (oy - sifrac) + nextBlue[xi] * sifrac);
            }
    ...
    else
    {
        ...
        for (yi = 0; yi < oy; yi++)
        {
            ...
            {
                GetScaledRow(
                    ImIn, x1, si, ix, ox, nextRed, nextGreen, nextBlue);
...
static XImage* ResizeImageSmooth(
    Display* dpy, XImage* Im, int ix, int iy, int ox, int oy)
{
    Visual* visual = DefaultVisual(dpy, DefaultScreen(dpy));
    ...
    Tmp = XCreateImage(
        dpy, visual, Im->depth, ZPixmap, 0, Data, width2, height2, 32, 0);
