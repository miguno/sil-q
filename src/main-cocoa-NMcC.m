...
/* We know how to double-buffer either into a CGLayer or a CGImage.  Currently
 * CGLayers are a lot faster, probably because NSImage thrashes between a
 * CGImageRef and a bitmap context. */
...
/* Whether or not we allow sounds (only relevant for the screensaver, where the
 * user can't configure it in-game) */
...
/* An AngbandContext represents a logical Term (i.e. what Angband thinks is a
 * window). This typically maps to one NSView, but may map to more than one
...
/* Returns the scale factor, that is, the scaling between our base logical
...
/* Locks focus on the Angband image but does NOT scale the CTM. Appropriate for
 * drawing hairlines. */
...
/* Descriptions of how to "push pixels" in a given rect to integralize. For
 * example, PUSH_LEFT means that we round expand the left edge if set, otherwise
 * we shrink it. */
enum
{
    PUSH_LEFT = 0x1,
...
{
    double rightPixel
        = push_pixel(NSMaxX(rect), scale.width, !!(pushOptions & PUSH_RIGHT));
    double topPixel
        = push_pixel(NSMaxY(rect), scale.height, !!(pushOptions & PUSH_TOP));
    double leftPixel
        = push_pixel(NSMinX(rect), scale.width, !(pushOptions & PUSH_LEFT));
    double bottomPixel
        = push_pixel(NSMinY(rect), scale.height, !(pushOptions & PUSH_BOTTOM));
    return NSMakeRect(
        leftPixel, bottomPixel, rightPixel - leftPixel, topPixel - bottomPixel);
}
...
//{
//   return current_graphics_mode && current_graphics_mode->grafID !=
//   GRAPHICS_NONE;
// }
...
- (NSSize)baseSize
{
     ...
     * the border. */
    return NSMakeSize(floor(cols * tileSize.width + 2 * borderSize.width),
        floor(rows * tileSize.height + 2 * borderSize.height));
}
...
#endif
        return NSMakeSize(currentSize.width / baseSize.width,
...
- (void)updateGlyphInfo
{
    ...
    for (i = 0; i < GLYPH_COUNT; i++)
        latinString[i] = (unsigned char)i;

    // Turn that into unichar. Angband uses ISO Latin 1.
    unichar unicharString[GLYPH_COUNT] = { 0 };
    NSString* allCharsString =
        [[NSString alloc] initWithBytes:latinString
                                 length:sizeof latinString
                               encoding:NSISOLatin1StringEncoding];
    [allCharsString
        getCharacters:unicharString
                range:NSMakeRange(
                          0, MIN(GLYPH_COUNT, [allCharsString length]))];
    [allCharsString autorelease];

    // Get glyphs
    bzero(glyphArray, sizeof glyphArray);
    CTFontGetGlyphsForCharacters(
        (CTFontRef)screenFont, unicharString, glyphArray, GLYPH_COUNT);

    // Get advances. Record the max advance.
    CGSize advances[GLYPH_COUNT] = {};
    CTFontGetAdvancesForGlyphs((CTFontRef)screenFont,
        kCTFontHorizontalOrientation, glyphArray, advances, GLYPH_COUNT);
    for (i = 0; i < GLYPH_COUNT; i++)
    {
...
- (void)updateImage
{
    ...
    if (activeView)
    {
         ...
         * nicely to any size. If we are not in live resize, then use the bounds
         * of the active view. */
        NSScreen* screen;
        if ([self useLiveResizeOptimization]
            && (screen = [[activeView window] screen]) != NULL)
        {
    ...
    CGContextRef exampleCtx = CGBitmapContextCreate(NULL, 1, 1,
        8 /* bits per component */, 48 /* bytesPerRow */, cs,
...
- (void)viewWillStartLiveResize:(AngbandView*)view
{
    ...
    if (inLiveResize < INT_MAX)
        inLiveResize++;
    else
        [NSException raise:NSInternalInconsistencyException
                    format:@"inLiveResize overflow"];

    if (inLiveResize == 1)
    {
        [self updateImage];

        [self setNeedsDisplay:YES]; // we'll need to redisplay everything
                                    // anyways, so avoid creating all those
                                    // little redisplay rects
        [self requestRedraw];
    }
#endif
}
...
- (void)throttle
{
    if (frames_per_second > 0)
    {
        ...
        CFTimeInterval timeUntilNextRefresh
            = (1. / (double)frames_per_second) - timeSinceLastRefresh;
...
- (void)drawRect:(NSRect)rect inView:(NSView*)view
{
    ...
    [angbandImage drawInRect:[view bounds]
                    fromRect:NSZeroRect
...
- (void)drawWChar:(wchar_t)wchar inRect:(NSRect)tile
{
    ...
    bzero(advances, 1);
    CTFontGetGlyphsForCharacters(
        (CTFontRef)screenFont, unicharString, thisGlyphArray, 1);
    CGGlyph glyph = thisGlyphArray[0];
    CTFontGetAdvancesForGlyphs((CTFontRef)screenFont,
        kCTFontHorizontalOrientation, thisGlyphArray, advances, 1);
     ...
     * need to compress it horizontally. Compute the compression ratio. 1.0
...
{
    bzero(
        charOverdrawCache, self->cols * self->rows * sizeof *charOverdrawCache);
    bzero(
        attrOverdrawCache, self->cols * self->rows * sizeof *attrOverdrawCache);
}
...
- (NSRect)rectInImageForTileAtX:(int)x Y:(int)y
{
    ...
    return NSMakeRect(x * tileSize.width + borderSize.width,
        flippedY * tileSize.height + borderSize.height, tileSize.width,
        tileSize.height);
}
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        self->charOverdrawCache = NSAllocateCollectable(
            self->cols * self->rows * sizeof *charOverdrawCache, 0);
        self->attrOverdrawCache = NSAllocateCollectable(
            self->cols * self->rows * sizeof *attrOverdrawCache, 0);

        /* Allocate our array of views */
        angbandViews = [[NSMutableArray alloc] init];

        /* Make the image. Since we have no views, it'll just be a puny 1x1
         * image. */
        [self updateImage];
    }
    return self;
...
- (void)dispose
{
    ...
    [angbandViews makeObjectsPerformSelector:@selector(setAngbandContext:)
                                  withObject:nil];
    ...
    if (![NSGarbageCollector defaultCollector])
        free(self->charOverdrawCache);
    self->charOverdrawCache = NULL;
    if (![NSGarbageCollector defaultCollector])
        free(self->attrOverdrawCache);
 ...
 * Preferences, we don't want to keep driving the AngbandView in the background.
 * Our active AngbandView is the widest - that's a hack all right. Mercifully
 * when we're just playing the game there's only one view. */
- (AngbandView*)activeView
{
...
static NSMenuItem* superitem(NSMenuItem* self)
{
    ...
    if (index == -1)
        ...
        return [supermenu itemAtIndex:index];
}
...
- (NSWindow*)makePrimaryWindow
{
    if (!primaryWindow)
    {
        /* The base size is quite small, so double its width and height as an
         * initial guess */
        ...
        AngbandView* angbandView =
            [[AngbandView alloc] initWithFrame:contentRect];
        ...
        /* Update our image, since this is probably the first angband view we've
         * gotten. */
        [self updateImage];
    }
...
- (void)setNeedsDisplayInBaseRect:(NSRect)rect
{
    for (NSView* angbandView in angbandViews)
    {
        [angbandView setNeedsDisplayInRect:[self convertBaseRect:rect
                                                          toView:angbandView]];
    }
}
...
- (NSSize)scaleFromBaseSize
{
    ...
    return NSMakeSize(
        boundsSize.width / baseSize.width, boundsSize.height / baseSize.height);
}
...
static void set_color_for_index(int idx)
{
    ...
    CGContextSetRGBFillColor([[NSGraphicsContext currentContext] graphicsPort],
        rv / 255., gv / 255., bv / 255., 1.);
}

/* Remember the current character in UserDefaults so we can select it by default
 * next time. */
static void record_current_savefile(void)
{
    NSString* savefileString = [[NSString
        stringWithCString:savefile
                 encoding:NSMacOSRomanStringEncoding] lastPathComponent];
    if (savefileString)
    {
...
static void Term_init_cocoa(term* t)
{
    ...
    if (!fontSize)
        fontSize = [default_font pointSize];
     ...
     * FullScreenAuxilliary, so they can at least show up. Unfortunately in Lion
     * they don't get brought to the full screen space; but they would only make
     * sense on multiple displays anyways so it's not a big loss. */
     ...
     * front from update_term_visibility(). This is to work around a problem
     * where Angband aggressively tells us to initialize terms that don't do
     * anything! */
    if (t == angband_term[0])
        [context orderFront];
...
static CGImageRef create_angband_image(NSString* name)
{
    ...
    if (path)
    {
        ...
        if (url)
        {
            NSDictionary* options =
                [[NSDictionary alloc] initWithObjectsAndKeys:(id)kCFBooleanTrue,
            ...
            if (source)
            {
                ...
                decodedImage = CGImageSourceCreateImageAtIndex(
    ...
    if (decodedImage)
    {
        size_t width = CGImageGetWidth(decodedImage),
               height = CGImageGetHeight(decodedImage);
        ...
        CGContextRef ctx = CGBitmapContextCreate(NULL, width, height,
            CGImageGetBitsPerComponent(decodedImage),
            CGImageGetBytesPerRow(decodedImage),
            CGImageGetColorSpace(decodedImage), contextBitmapInfo);
...
static errr Term_xtra_cocoa(int n, int v)
{
    ...
    case TERM_XTRA_LEVEL:
    {
         ...
         * should be telling us our window order (the user should decide that),
         * so do nothing. */
        break;
    }
...
static errr Term_curs_cocoa(int x, int y)
{
    ...
    /* Scale the rect. Normally lockFocus would set up the CTM for us, but here
     * we're calling lockFocusUnscaled so we can have a uniform cursor size
     * (e.g. 1 px) regardless of the scale factor. */
...
static errr Term_wipe_cocoa(int x, int y, int n)
{
    ...
    if (n > 1)
        rect = NSUnionRect(
            rect, [angbandContext rectInImageForTileAtX:x + n - 1 Y:y]);
...
static void draw_image_tile(
    CGImageRef image, NSRect srcRect, NSRect dstRect, NSCompositingOperation op)
{
    ...
    CGImageRef subimage
        = CGImageCreateWithImageInRect(image, *(CGRect*)&srcRect);
...
static errr Term_text_cocoa(int x, int y, int n, byte a, const char* cp)
{
    ...
    /* Subpixel rendering looks really nice!  Unfortunately, drawing a string
     like this:
     .@
     causes subpixels to extend slightly into the region 'owned' by the period.
     This means that when the user presses right, those subpixels 'owned' by the
     period above do not get redrawn by Angband, so we leave little blue and red
     subpixel turds all over the screen.  Turning off subpixel rendering fixes
     this, as does increasing the font advance by a pixel, but that is ugly. Our
     hack solution is to remember all of the characters we draw as well as their
     locations and colors (in charOverdrawCache), and then re-blit the previous
     and next character (if any).
     */
    ...
    NSRectFill(crack_rect(
        rectToClear, scaleFactor, leftPushOptions | rightPushOptions));
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {
            wchar_t previouslyDrawnVal
                = angbandContext
                      ->charOverdrawCache[y * angbandContext->cols + overdrawX];
            // Don't overdraw if it's not text
            if (previouslyDrawnVal != NO_OVERDRAW)
            {
                NSRect overdrawRect =
                    [angbandContext rectInImageForTileAtX:overdrawX Y:y];
                NSRect expandedRect = crack_rect(
                    overdrawRect, scaleFactor, push_options(overdrawX, y));
                ...
                if (previouslyDrawnVal != 0)
                {
                    byte color
                        = angbandContext
                              ->attrOverdrawCache[y * angbandContext->cols
                                  + overdrawX];

                    set_color_for_index(color);
                    [angbandContext drawWChar:previouslyDrawnVal
                                       inRect:overdrawRect];
                }
            }
        }
...
static void load_prefs()
{
    ...
    /* fps */
    frames_per_second =
        [[NSUserDefaults angbandDefaults] integerForKey:@"FramesPerSecond"];

    /* font */
    default_font =
        [[NSFont fontWithName:[defs valueForKey:@"FontName-0"]
                         size:[defs floatForKey:@"FontSize-0"]] retain];
    if (!default_font)
        default_font = [[NSFont fontWithName:@"Menlo" size:13.] retain];
}
...
{
    if ([view isKindOfClass:[AngbandView class]])
        return YES;
    for (NSView* subview in [view subviews])
    {
        if (contains_angband_view(subview))
            return YES;
    }
    return NO;
}
...
static BOOL send_event(NSEvent* event)
{
    ...
    case NSKeyDown:
    {
        ...
        if (ch != '\0')
        {
            ...
            if (mx)
                mods |= KC_MOD_META;
            if (mc && MODS_INCLUDE_CONTROL(ch))
                mods |= KC_MOD_CONTROL;
            if (ms && MODS_INCLUDE_SHIFT(ch))
                mods |= KC_MOD_SHIFT;
            if (kp)
                mods |= KC_MOD_KEYPAD;
...
/* Update window visibility to match what's in p_ptr, so we show or hide terms
 * that have or do not have contents respectively. */
static void update_term_visibility(void)
{
    if (!op_ptr)
        return; // paranoia
    ...
    for (i = 0; i < ANGBAND_TERM_MAX; i++)
    {
        ...
        if (t)
        {
            ...
            /* Ensure the first term is always visible. The remaining terms
             * depend on the op_ptr. */
            BOOL shouldBeVisible = (i == 0
                || (op_ptr->window_flag[i] & significantWindowFlagMask));
...
static void hook_plog(const char* str)
{
    ...
    {
        NSString* string =
            [NSString stringWithCString:str
                               encoding:NSMacOSRomanStringEncoding];
        NSRunAlertPanel(
            @"Danger Will Robinson", @"%@", @"OK", nil, nil, string);
    }
}
    ...
    if (pathString)
    {
        NSDictionary* attrs = [NSDictionary
            dictionaryWithObjectsAndKeys:[NSNumber
                                             numberWithUnsignedLong:ftype],
            NSFileHFSTypeCode,
            [NSNumber numberWithUnsignedLong:ANGBAND_CREATOR],
            NSFileHFSCreatorCode, nil];
...
static void initialize_file_paths(void)
{
    ...
    NSString* libString = [[[NSBundle bundleForClass:[AngbandView class]]
        resourcePath] stringByAppendingPathComponent:@"/lib"];
    BOOL isDir = NO;
    if (![fm fileExistsAtPath:libString isDirectory:&isDir] || !isDir)
    {
        NSRunAlertPanel(@"Unable to find lib directory",
            @"Unable to find the lib directory at path %@.  Angband has to "
            @"quit.",
            @"Nuts", nil, nil, libpath);
        exit(0);
    }
    ...
    NSString* angbandBase = libString;
    [angbandBase getFileSystemRepresentation:basepath
                                   maxLength:sizeof basepath];
    ...
    BOOL success = YES;
    success = success &&
        [fm createDirectoryAtPath:config
            withIntermediateDirectories:YES
                             attributes:nil
                                  error:&error];
    success = success &&
        [fm createDirectoryAtPath:save
            withIntermediateDirectories:YES
                             attributes:nil
                                  error:&error];
    success = success &&
        [fm createDirectoryAtPath:user
            withIntermediateDirectories:YES
                             attributes:nil
                                  error:&error];
    if (!success)
    {
        NSRunAlertPanel(@"Unable to create directory",
            @"Unable to create directory in %@ (error was %@).  Angband has to "
            @"quit.",
            @"Nuts", nil, nil, angbandBase, error);
        [[NSApplication sharedApplication] presentError:error];
        exit(0);
    }

    // void init_file_paths(const char *configpath, const char *libpath, const
    // char *datapath)
...
- (IBAction)openGame:sender
{
    ...
    startingDirectory =
        [get_data_directory() stringByAppendingPathComponent:@"/save/"];

    /* Get what we think the default save file name is. Deafult to the empty
     * string. */
    ...
    /* Run it */
    panelResult = [panel runModalForDirectory:startingDirectory
                                         file:savefileName
                                        types:nil];
...
- (IBAction)setRefreshRate:(NSMenuItem*)menuItem
{
    ...
    [[NSUserDefaults angbandDefaults] setInteger:frames_per_second
...
- (void)applicationDidFinishLaunching:sender
{
    ...
    // once beginGame finished, the game is over - that's how Angband works, and
    // we should quit
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication*)sender
{
    ...
    else
    {
        ...
        wakeup_event_loop();
        quit_when_ready = true;
        // must return Cancel, not Later, because we need to get out of the run
        // loop and back to Angband's loop
...
- (void)menuNeedsUpdate:(NSMenu*)menu
{
    ...
    NSMenuItem* item = [menu addItemWithTitle:@"Classic ASCII"
                                       action:action
                                keyEquivalent:@""];
