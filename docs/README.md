...
On macOS you can install pandoc via `just install` (uses `Brewfile`). On
other systems, install pandoc and a TeX distribution with your package
manager.
...
- The PDF build consumes `manual.md` as-is via the Eisvogel LaTeX template
  in [pandoc/templates/eisvogel/](pandoc/templates/eisvogel/).
